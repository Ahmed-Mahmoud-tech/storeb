...
export class UserActionService {
  ...
  async recordAction(
    ...
    console.error(`Recording action: ${createUserActionDto.action_type} for user: ${userId}`);
    ...
    try {
      ...
      console.error(`Created user action object: ${JSON.stringify(userAction)}`);
      this.logger.log(`Created user action object: ${JSON.stringify(userAction)}`);
  ...
  async getUserActions(
    ...
    const { action_type, store_id, product_id, start_date, end_date, limit = '50', offset = '0' } = query;
    ...
    if (start_date && end_date) {
      whereClause.created_at = Between(new Date(start_date), new Date(end_date));
    }
  ...
  async getStoreActions(
    ...
    const { action_type, start_date, end_date, limit = '50', offset = '0' } = query;
